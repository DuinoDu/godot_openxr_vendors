...
#define _impl_XR_LIST_ALL_STRUCTURE_TYPES_CORE(_avail, _unavail) \
    ...
    _avail(XrMrcSpaceCreateInfoPICO, XR_TYPE_MRC_SPACE_CREATE_INFO_PICO) \
    _avail(XrExternalCameraParameterPICO, XR_TYPE_EXTERNAL_CAMERA_PARAMETER_PICO) \
    _avail(XrVirtualBoundaryInfoPICO, XR_TYPE_VIRTUAL_BOUNDARY_INFO_PICO) \
    ...
    _avail(XrVirtualBoundaryStatusPICO, XR_TYPE_VIRTUAL_BOUNDARY_STATUS_PICO) \
    _avail(XrVirtualBoundaryTriggerPICO, XR_TYPE_VIRTUAL_BOUNDARY_TRIGGER_PICO) \
    _avail(XrMotionTrackerSpaceVelocityPICO, XR_TYPE_MOTION_TRACKER_SPACE_VELOCITY_PICO) \
    _avail(XrMotionTrackerSpaceLocationPICO, XR_TYPE_MOTION_TRACKER_SPACE_LOCATION_PICO) \
    ...
    _avail(XrMotionTrackerBatteryStatePICO, XR_TYPE_MOTION_TRACKER_BATTERY_STATE_PICO) \
    _avail(XrMotionTrackerLocationInfoPICO, XR_TYPE_MOTION_TRACKER_LOCATION_INFO_PICO) \
    _avail(XrCompositionLayerFisheyePICO, XR_TYPE_COMPOSITION_LAYER_FISHEYE_PICO) \
    _avail(XrLayerSettingsPICO, XR_TYPE_LAYER_SETTINGS_PICO) \
    _avail(XrEyeTrackerCreateInfoPICO, XR_TYPE_EYE_TRACKER_CREATE_INFO_PICO) \
    _avail(XrEyeTrackerDataInfoPICO, XR_TYPE_EYE_TRACKER_DATA_INFO_PICO) \
    _avail(XrEyeDataPICO, XR_TYPE_EYE_DATA_PICO) \
    _avail(XrEyeTrackerDataPICO, XR_TYPE_EYE_TRACKER_DATA_PICO) \
    _avail(XrSecureMrFrameworkCreateInfoPICO, XR_TYPE_SECURE_MR_FRAMEWORK_CREATE_INFO_PICO) \
    _avail(XrSecureMrPipelineCreateInfoPICO, XR_TYPE_SECURE_MR_PIPELINE_CREATE_INFO_PICO) \
    _avail(XrSecureMrOperatorCreateInfoPICO, XR_TYPE_SECURE_MR_OPERATOR_CREATE_INFO_PICO) \
    _avail(XrSecureMrOperatorIOMapPICO, XR_TYPE_SECURE_MR_OPERATOR_IO_MAP_PICO) \
    ...
    _avail(XrSecureMrTensorBufferPICO, XR_TYPE_SECURE_MR_TENSOR_BUFFER_PICO) \
    ...
    _avail(XrSecureMrPipelineIOPairPICO, XR_TYPE_SECURE_MR_PIPELINE_IO_PAIR_PICO) \
    _avail(XrSecureMrPipelineExecuteParameterPICO, XR_TYPE_SECURE_MR_PIPELINE_EXECUTE_PARAMETER_PICO) \
    ...
    _avail(XrExpandDeviceMotorVibratePICO, XR_TYPE_EXPAND_DEVICE_MOTOR_VIBRATE_PICO) \
    _avail(XrExpandDeviceCustomDataPICO, XR_TYPE_EXPAND_DEVICE_CUSTOM_DATA_PICO) \
    _avail(XrExpandDeviceBatteryStatePICO, XR_TYPE_EXPAND_DEVICE_BATTERY_STATE_PICO) \
    ...
    _avail(XrBodyBoneLengthPICO, XR_TYPE_BODY_BONE_LENGTH_PICO) \
    _avail(XrBodyTrackingPostureFlagsDataPICO, XR_TYPE_BODY_TRACKING_POSTURE_FLAGS_DATA_PICO) \
    _avail(XrBodyJointVelocitiesPICO, XR_TYPE_BODY_JOINT_VELOCITIES_PICO) \
    _avail(XrBodyJointAccelerationsPICO, XR_TYPE_BODY_JOINT_ACCELERATIONS_PICO) \
    _avail(XrBodyTrackingStatePICO, XR_TYPE_BODY_TRACKING_STATE_PICO) \
    _avail(XrSymmetricFovReprojectionViewConfigurationPICO, XR_TYPE_SYMMETRIC_FOV_REPROJECTION_VIEW_CONFIGURATION_PICO) \
    _avail(XrLayerColorMatrixPICO, XR_TYPE_LAYER_COLOR_MATRIX_PICO) \
    _avail(XrReadbackTensorBufferPICO, XR_TYPE_READBACK_TENSOR_BUFFER_PICO) \
    ...
    _avail(XrReadbackTextureImageBasePICO, XR_TYPE_READBACK_TEXTURE_IMAGE_BASE_PICO) \
    _avail(XrSystemSpatialSensingPropertiesPICO, XR_TYPE_SYSTEM_SPATIAL_SENSING_PROPERTIES_PICO) \
    _avail(XrSpatialEntityLocationGetInfoPICO, XR_TYPE_SPATIAL_ENTITY_LOCATION_GET_INFO_PICO) \
    _avail(XrSpatialEntityLocationDataPICO, XR_TYPE_SPATIAL_ENTITY_LOCATION_DATA_PICO) \
    _avail(XrSpatialEntitySemanticGetInfoPICO, XR_TYPE_SPATIAL_ENTITY_SEMANTIC_GET_INFO_PICO) \
    _avail(XrSpatialEntitySemanticDataPICO, XR_TYPE_SPATIAL_ENTITY_SEMANTIC_DATA_PICO) \
    _avail(XrSpatialEntityBoundingBox2DGetInfoPICO, XR_TYPE_SPATIAL_ENTITY_BOUNDING_BOX_2D_GET_INFO_PICO) \
    _avail(XrSpatialEntityBoundingBox2DDataPICO, XR_TYPE_SPATIAL_ENTITY_BOUNDING_BOX_2D_DATA_PICO) \
    _avail(XrSpatialEntityPolygonGetInfoPICO, XR_TYPE_SPATIAL_ENTITY_POLYGON_GET_INFO_PICO) \
    _avail(XrSpatialEntityPolygonDataPICO, XR_TYPE_SPATIAL_ENTITY_POLYGON_DATA_PICO) \
    _avail(XrSpatialEntityBoundingBox3DGetInfoPICO, XR_TYPE_SPATIAL_ENTITY_BOUNDING_BOX_3D_GET_INFO_PICO) \
    _avail(XrSpatialEntityBoundingBox3DDataPICO, XR_TYPE_SPATIAL_ENTITY_BOUNDING_BOX_3D_DATA_PICO) \
    _avail(XrSpatialEntityTriangleMeshGetInfoPICO, XR_TYPE_SPATIAL_ENTITY_TRIANGLE_MESH_GET_INFO_PICO) \
    _avail(XrSpatialEntityTriangleMeshDataPICO, XR_TYPE_SPATIAL_ENTITY_TRIANGLE_MESH_DATA_PICO) \
    _avail(XrSenseDataProviderStartInfoPICO, XR_TYPE_SENSE_DATA_PROVIDER_START_INFO_PICO) \
    ...
    _avail(XrQueriedSenseDataGetInfoPICO, XR_TYPE_QUERIED_SENSE_DATA_GET_INFO_PICO) \
    _avail(XrSpatialEntityStatePICO, XR_TYPE_SPATIAL_ENTITY_STATE_PICO) \
    _avail(XrQueriedSenseDataPICO, XR_TYPE_QUERIED_SENSE_DATA_PICO) \
    ...
    _avail(XrSpatialEntityAnchorRetrieveInfoPICO, XR_TYPE_SPATIAL_ENTITY_ANCHOR_RETRIEVE_INFO_PICO) \
    _avail(XrAnchorLocateInfoPICO, XR_TYPE_ANCHOR_LOCATE_INFO_PICO) \
    _avail(XrFuturePollResultProgressPICO, XR_TYPE_FUTURE_POLL_RESULT_PROGRESS_PICO) \
    _avail(XrSystemSpatialAnchorPropertiesPICO, XR_TYPE_SYSTEM_SPATIAL_ANCHOR_PROPERTIES_PICO) \
    _avail(XrSenseDataProviderCreateInfoSpatialAnchorPICO, XR_TYPE_SENSE_DATA_PROVIDER_CREATE_INFO_SPATIAL_ANCHOR_PICO) \
    _avail(XrSpatialAnchorCreateInfoPICO, XR_TYPE_SPATIAL_ANCHOR_CREATE_INFO_PICO) \
    _avail(XrSpatialAnchorCreateCompletionPICO, XR_TYPE_SPATIAL_ANCHOR_CREATE_COMPLETION_PICO) \
    _avail(XrSpatialAnchorPersistInfoPICO, XR_TYPE_SPATIAL_ANCHOR_PERSIST_INFO_PICO) \
    _avail(XrSpatialAnchorPersistCompletionPICO, XR_TYPE_SPATIAL_ANCHOR_PERSIST_COMPLETION_PICO) \
    _avail(XrSpatialAnchorUnpersistInfoPICO, XR_TYPE_SPATIAL_ANCHOR_UNPERSIST_INFO_PICO) \
    _avail(XrSpatialAnchorUnpersistCompletionPICO, XR_TYPE_SPATIAL_ANCHOR_UNPERSIST_COMPLETION_PICO) \
    _avail(XrSystemSpatialAnchorSharingPropertiesPICO, XR_TYPE_SYSTEM_SPATIAL_ANCHOR_SHARING_PROPERTIES_PICO) \
    _avail(XrSpatialAnchorShareInfoPICO, XR_TYPE_SPATIAL_ANCHOR_SHARE_INFO_PICO) \
    _avail(XrSpatialAnchorShareCompletionPICO, XR_TYPE_SPATIAL_ANCHOR_SHARE_COMPLETION_PICO) \
    _avail(XrSharedSpatialAnchorDownloadInfoPICO, XR_TYPE_SPATIAL_ANCHOR_DOWNLOAD_INFO_PICO) \
    _avail(XrSharedSpatialAnchorDownloadCompletionPICO, XR_TYPE_SPATIAL_ANCHOR_DOWNLOAD_COMPLETION_PICO) \
    _avail(XrSystemSceneCapturePropertiesPICO, XR_TYPE_SYSTEM_SCENE_CAPTURE_PROPERTIES_PICO) \
    _avail(XrSenseDataProviderCreateInfoSceneCapturePICO, XR_TYPE_SENSE_DATA_PROVIDER_CREATE_INFO_SCENE_CAPTURE_PICO) \
    _avail(XrSceneCaptureStartInfoPICO, XR_TYPE_SCENE_CAPTURE_START_INFO_PICO) \
    _avail(XrSceneCaptureStartCompletionPICO, XR_TYPE_SCENE_CAPTURE_START_COMPLETION_PICO) \
    _avail(XrSystemSpatialMeshPropertiesPICO, XR_TYPE_SYSTEM_SPATIAL_MESH_PROPERTIES_PICO) \
